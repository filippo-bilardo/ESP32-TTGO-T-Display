...
uint8_t espnow_macaddr[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
void L1_EspNowInit() {
  ...
  // Register peer
  memcpy(peerInfo.peer_addr, espnow_macaddr, 6);
  peerInfo.channel = 0;  
...
void L1_EspNowTx(uint8_t *data, size_t len) {
  ...
  //esp_err_t result = esp_now_send(espnow_macaddr, (uint8_t *)&myData, sizeof(myData));
  esp_err_t result = esp_now_send(espnow_macaddr, data, len);
  if (result == ESP_OK) {
